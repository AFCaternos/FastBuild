...
function showChunkExportUI() {
    runOnUiThread(function() {
        try {
            ...
            // ── 坐标偏离 ──
            var offsetLabel = createLabel("坐标偏离 (可输入负数)", 13, FB.colors.textSecondary, false);
            if (offsetLabel !== null) { offsetLabel.setLayoutParams(mp(0, 12, 0, 0)); main.addView(offsetLabel); }
            
            var offsetRow = new android.widget.LinearLayout(ctx);
            offsetRow.setOrientation(0);
            var offXInput = createNumInput("X偏移", "0");
            var offYInput = createNumInput("Y偏移", "0");
            var offZInput = createNumInput("Z偏移", "0");
            if (offXInput !== null) { offXInput.setLayoutParams(new android.widget.LinearLayout.LayoutParams(0, -2, 1)); offsetRow.addView(offXInput); }
            if (offYInput !== null) { offYInput.setLayoutParams(new android.widget.LinearLayout.LayoutParams(0, -2, 1)); offsetRow.addView(offYInput); }
            if (offZInput !== null) { offZInput.setLayoutParams(new android.widget.LinearLayout.LayoutParams(0, -2, 1)); offsetRow.addView(offZInput); }
            main.addView(offsetRow);
            
            // ── 旋转选择 ──
            var rotLabel = createLabel("旋转角度", 13, FB.colors.textSecondary, false);
            if (rotLabel !== null) { rotLabel.setLayoutParams(mp(0, 12, 0, 0)); main.addView(rotLabel); }
            
            var rotRg = new android.widget.RadioGroup(ctx);
            rotRg.setOrientation(0);
            
            var rot0 = new android.widget.RadioButton(ctx);
            rot0.setText(" 0°"); rot0.setTextSize(13); rot0.setId(0); rot0.setChecked(true); applyRadioTint(rot0);
            rotRg.addView(rot0);
            
            var rot90 = new android.widget.RadioButton(ctx);
            rot90.setText(" 90°"); rot90.setTextSize(13); rot90.setId(90); applyRadioTint(rot90);
            rotRg.addView(rot90);
            
            var rot180 = new android.widget.RadioButton(ctx);
            rot180.setText(" 180°"); rot180.setTextSize(13); rot180.setId(180); applyRadioTint(rot180);
            rotRg.addView(rot180);
            
            var rot270 = new android.widget.RadioButton(ctx);
            rot270.setText(" 270°"); rot270.setTextSize(13); rot270.setId(270); applyRadioTint(rot270);
            rotRg.addView(rot270);
            
            var rot360 = new android.widget.RadioButton(ctx);
            rot360.setText(" 360°"); rot360.setTextSize(13); rot360.setId(360); applyRadioTint(rot360);
            rotRg.addView(rot360);
            
            main.addView(rotRg);
            
            // ── 包含起点 ──
            var originCheck = new android.widget.CheckBox(ctx);
            originCheck.setText(" 包含起点坐标方块");
            originCheck.setTextSize(14);
            originCheck.setTextColor(android.graphics.Color.parseColor(FB.colors.textPrimary));
            originCheck.setChecked(true);
            originCheck.setLayoutParams(mp(0, 12, 0, 0));
            applyCheckBoxTint(originCheck);
            main.addView(originCheck);
            ...
            var exportBtn = createBtn("🚀  开始导出", FB.colors.primary, FB.colors.primaryDark, function() {
                try {
                    ...
                    // 偏离
                    var oX = parseInt(offXInput.getText()) || 0;
                    var oY = parseInt(offYInput.getText()) || 0;
                    var oZ = parseInt(offZInput.getText()) || 0;
                    
                    // 旋转
                    var rotation = rotRg.getCheckedRadioButtonId();
                    
                    // 包含起点
                    var incOrigin = originCheck.isChecked();
                    
                    if (FB.ui.mainWindow !== null) { FB.ui.mainWindow.dismiss(); FB.ui.mainWindow = null; }
                    startChunkExport(name, range, yMin, yMax, air, {x: oX, y: oY, z: oZ}, rotation, incOrigin);
                } catch (e) {}
            });
        ...
        try {
            ...
            var exportBtn = createBtn("🚀  开始导出", FB.colors.primary, FB.colors.primaryDark, function() {
                try {
                    ...
                    if (FB.ui.mainWindow !== null) { FB.ui.mainWindow.dismiss(); FB.ui.mainWindow = null; }
                    startCoordExport(name, px1, py1, pz1, px2, py2, pz2, air, {x: oX, y: oY, z: oZ}, rotation, incOrigin);
                } catch (e) {}
            });
            if (exportBtn !== null) main.addView(exportBtn);
...
function startChunkExport(fileName, range, yMin, yMax, includeAir, offsetXYZ, rotation, includeOrigin) {
    ...
    executeExport(fileName, x1, yMin, z1, x2, yMax, z2, includeAir, offsetXYZ, rotation, includeOrigin);
}

function startCoordExport(fileName, x1, y1, z1, x2, y2, z2, includeAir, offsetXYZ, rotation, includeOrigin) {
    if (FB.state.isProcessing) { toast("有任务进行中"); return; }
    ...
    executeExport(fileName, minX, minY, minZ, maxX, maxY, maxZ, includeAir, offsetXYZ, rotation, includeOrigin);
}
...
// 绕 Y 轴顺时针旋转方块坐标
function rotateBlockCoord(bx, by, bz, rotation, sizeX, sizeZ) {
    switch (rotation) {
        case 90:
            return { x: bz, y: by, z: sizeX - 1 - bx };
        case 180:
            return { x: sizeX - 1 - bx, y: by, z: sizeZ - 1 - bz };
        case 270:
            return { x: sizeZ - 1 - bz, y: by, z: bx };
        default:
...
// 获取旋转后的尺寸
function getRotatedSize(sizeX, sizeZ, rotation) {
...
function executeExport(fileName, x1, y1, z1, x2, y2, z2, includeAir, offsetXYZ, rotation, includeOrigin) {
    ...
    // 解析偏离
    var offX = (offsetXYZ && offsetXYZ.x) ? offsetXYZ.x : 0;
    var offY = (offsetXYZ && offsetXYZ.y) ? offsetXYZ.y : 0;
    var offZ = (offsetXYZ && offsetXYZ.z) ? offsetXYZ.z : 0;

    // 解析旋转
    var rot = (rotation === 90 || rotation === 180 || rotation === 270 || rotation === 360) ? rotation : 0;
    if (rot === 360) rot = 0;

    // 解析包含起点
    var incOrigin = (includeOrigin === false) ? false : true;

    // 旋转后尺寸
    var rotSize = getRotatedSize(sizeX, sizeZ, rot);
    var finalSizeX = rotSize.x;
    var finalSizeZ = rotSize.z;
        ...
        run: function() {
            ...
            try {
                ...
                if (FB.config.streamExport) {
                    ...
                    writer.write("\"origin\":{\"x\":" + originX + ",\"y\":" + originY + ",\"z\":" + originZ + "},");
                    writer.write("\"size\":{\"x\":" + finalSizeX + ",\"y\":" + sizeY + ",\"z\":" + finalSizeZ + "},");
                    writer.write("\"rotation\":" + rot + ",");
                    writer.write("\"blocks\":[");
                        ...
                        for (var z = z1; z <= z2 && FB.state.isProcessing; z++) {
                            for (var x = x1; x <= x2 && FB.state.isProcessing; x++) {
                                ...
                                if (blockId !== 0 || includeAir) {
                                    ...
                                    // 排除起点
                                    if (!incOrigin && bx === 0 && by === 0 && bz === 0) {
                                        processedBlocks++;
                                        continue;
                                    }

                                    // 应用旋转
                                    var rb = rotateBlockCoord(bx, by, bz, rot, sizeX, sizeZ);

                                    if (!first) writer.write(",");
                                    first = false;

                                    writer.write("{\"x\":" + rb.x + ",\"y\":" + rb.y + ",\"z\":" + rb.z +
                        ...
                        for (var zz = z1; zz <= z2 && FB.state.isProcessing; zz++) {
                            for (var xx = x1; xx <= x2 && FB.state.isProcessing; xx++) {
                                ...
                                if (id0 !== 0 || includeAir) {
                                    ...
                                    // 排除起点
                                    if (!incOrigin && rawX === 0 && rawY === 0 && rawZ === 0) {
                                        processedBlocks++;
                                        continue;
                                    }

                                    // 应用旋转
                                    var rb2 = rotateBlockCoord(rawX, rawY, rawZ, rot, sizeX, sizeZ);
                                    blocks.push({x: rb2.x, y: rb2.y, z: rb2.z, id: id0, data: data0});
                                    blockCount++;
                                }
                    ...
                    var dataObj = {
                        ...
                        size: {x: finalSizeX, y: sizeY, z: finalSizeZ},
                        rotation: rot,
                        blocks: blocks,
                ...
                clientMessage("§7方块: §e" + blockCount + " §7| 尺寸: §e" + finalSizeX + "x" + sizeY + "x" + finalSizeZ);
                clientMessage("§7大小: §e" + formatSize(fileSize));
                if (rot !== 0) clientMessage("§7旋转: §e" + rot + "°");
                if (offX !== 0 || offY !== 0 || offZ !== 0) clientMessage("§7偏移: §e" + offX + ", " + offY + ", " + offZ);
                if (!incOrigin) clientMessage("§7已排除起点方块");
